...
local function lrucache_enable(size)
    local size = size or 4000  -- Cache the last 4000 IPs (~1MB memory) by default

    --local lrucache_obj, err = require("resty.lua_resty_lrucache.lrucache").new(size)
    local lrucache_require = require("resty.lua_resty_lrucache.lrucache")
    local lrucache_obj, err = lrucache_require.new(size)
...
_M.lrucache_enable = lrucache_enable
